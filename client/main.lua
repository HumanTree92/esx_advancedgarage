...
AddEventHandler(
    "esx_advancedgarage:getPropertiesC",
    function(xPlayer)
        if Config.UsePrivateCarGarages then
            ESX.TriggerServerCallback(
                "esx_advancedgarage:getOwnedProperties",
...
function ListOwnedAmbulanceMenu()
    ...
    if Config.ShowVehicleLocation and Config.ShowSpacers then
        local spacer =
            ('| <span style="color:red;">%s</span> - <span style="color:darkgoldenrod;">%s</span> - <span style="color:red;">%s</span> |'):format(
            _U("plate"),
            _U("vehicle"),
            _U("location")
        )
    ...
    elseif Config.ShowVehicleLocation == false and Config.ShowSpacers then
        local spacer =
            ('| <span style="color:red;">%s</span> - <span style="color:darkgoldenrod;">%s</span> |'):format(
            _U("plate"),
            _U("vehicle")
        )
    ...
    ESX.TriggerServerCallback(
        "esx_advancedgarage:getOwnedAmbulanceCars",
        function(ownedAmbulanceCars)
            if #ownedAmbulanceCars == 0 then
                ...
                for _, v in pairs(ownedAmbulanceCars) do
                    ...
                    if Config.ShowVehicleLocation then
                        if v.stored then
                            labelvehicle =
                                labelvehicle2 .. ('<span style="color:green;">%s</span> |'):format(_U("loc_garage"))
                        else
                            labelvehicle =
                                labelvehicle2 .. ('<span style="color:red;">%s</span> |'):format(_U("loc_pound"))
                        end
                    else
                        if v.stored then
            ...
            ESX.TriggerServerCallback(
                "esx_advancedgarage:getOwnedAmbulanceAircrafts",
                function(ownedAmbulanceAircrafts)
                ...
                if valid then
                    if engineHealth < 990 then
                        if Config.UseDamageMult then
                            local apprasial =
                                math.floor(
                                (1000 - engineHealth) / 1000 * Config.AmbulancePoundPrice * Config.DamageMult
                            )
...
function ListOwnedPoliceMenu()
    ...
    ESX.TriggerServerCallback(
        "esx_advancedgarage:getOwnedPoliceCars",
        function(ownedPoliceCars)
            ...
            ESX.TriggerServerCallback(
                "esx_advancedgarage:getOwnedPoliceAircrafts",
                function(ownedPoliceAircrafts)
            ...
            function(valid)
                if valid then
                    if engineHealth < 990 then
                        if Config.UseDamageMult then
                            local apprasial =
                                math.floor((1000 - engineHealth) / 1000 * Config.PolicePoundPrice * Config.DamageMult)
                            RepairVehicle(apprasial, vehicle, vehicleProps)
                        else
...
function ReturnOwnedPoliceMenu()
    if WasinJPound then
        ...
        ESX.TriggerServerCallback(
            "esx_advancedgarage:getOutOwnedPoliceCars",
            function(ownedPoliceCars)
                local elements = {}
                ...
                ESX.UI.Menu.Open(
                    ...
                    function(data, menu)
                        ...
                        if not doesVehicleExist and not DoesAPlayerDrivesVehicle(data.current.value.plate) then
                            ESX.TriggerServerCallback(
                                "esx_advancedgarage:checkMoneyPolice",
                                function(hasEnoughMoney)
                                    if hasEnoughMoney then
                                        if data.current.value == nil then
                                        else
            ...
            function(valid)
                if valid then
                    if engineHealth < 990 then
                        if Config.UseDamageMult then
                            local apprasial =
                                math.floor((1000 - engineHealth) / 1000 * Config.AircraftPoundPrice * Config.DamageMult)
                            RepairVehicle(apprasial, vehicle, vehicleProps)
                        else
            ...
            function(valid)
                if valid then
                    if engineHealth < 990 then
                        if Config.UseDamageMult then
                            local apprasial =
                                math.floor((1000 - engineHealth) / 1000 * Config.BoatPoundPrice * Config.DamageMult)
                            RepairVehicle(apprasial, vehicle, vehicleProps)
                        else
                ...
                function(valid)
                    if valid then
                        if engineHealth < 990 then
                            if Config.UseDamageMult then
                                local apprasial =
                                    math.floor((1000 - engineHealth) / 1000 * Config.CarPoundPrice * Config.DamageMult)
                                RepairVehicle(apprasial, vehicle, vehicleProps, fuelLevel)
                            else
                                local apprasial = math.floor((1000 - engineHealth) / 1000 * Config.CarPoundPrice)
                                RepairVehicle(apprasial, vehicle, vehicleProps, fuelLevel)
                            end
                        else
                            StoreVehicle(vehicle, vehicleProps, fuelLevel)
                        end
                    else
                        ESX.ShowNotification(_U("cannot_store_vehicle"))
                    end
...
function SpawnVehicle(vehicle, plate)
    ESX.Game.SpawnVehicle(
        ...
        function(callback_vehicle)
            ...
            if Config.LegacyFuel then
                local tnkFuel = exports["LegacyFuel"]:SetFuel(callback_vehicle, fuelLevel)
            end
...
function SpawnVehicle2(vehicle, plate)
    ESX.Game.SpawnVehicle(
        vehicle.model,
        this_Garage.Spawner2,
        this_Garage.Heading2,
        function(callback_vehicle)
            ESX.Game.SetVehicleProperties(callback_vehicle, vehicle)
            SetVehRadioStation(callback_vehicle, "OFF")
            SetVehicleFixed(callback_vehicle)
            ...
            --SetVehicleBodyHealth(callback_vehicle, 1000) -- Might not be needed
            if Config.LegacyFuel then
                local tnkFuel = exports["LegacyFuel"]:GetFuel(callback_vehicle)
            end
            local carplate = GetVehicleNumberPlateText(callback_vehicle)
            table.insert(vehInstance, {vehicleentity = callback_vehicle, plate = carplate})
            TaskWarpPedIntoVehicle(GetPlayerPed(-1), callback_vehicle, -1)
                        ...
                        if distance < Config.DrawDistance then
                            ...
                            if Config.PointMarker.Type ~= -1 then
                                DrawMarker(
                                    ...
                                    0.0,
                                    0.0,
                                    Config.PointMarker.x,
                                    Config.PointMarker.y,
                                    Config.PointMarker.z,
                                    Config.PointMarker.r,
                                    Config.PointMarker.g,
                                    Config.PointMarker.b,
                                    100,
                                    false,
                                    true,
                                    2,
                        ...
                        if distance2 < Config.DrawDistance then
                            ...
                            if Config.DeleteMarker.Type ~= -1 then
                                DrawMarker(
                                    ...
                                    0.0,
                                    0.0,
                                    Config.DeleteMarker.x,
                                    Config.DeleteMarker.y,
                                    Config.DeleteMarker.z,
                                    Config.DeleteMarker.r,
                                    Config.DeleteMarker.g,
                                    Config.DeleteMarker.b,
                                    100,
                                    false,
                                    true,
                                    2,
                        ...
                        if distance < Config.DrawDistance then
                            ...
                            if Config.JPoundMarker.Type ~= -1 then
                                DrawMarker(
                                    ...
                                    0.0,
                                    0.0,
                                    Config.JPoundMarker.x,
                                    Config.JPoundMarker.y,
                                    Config.JPoundMarker.z,
                                    Config.JPoundMarker.r,
                                    Config.JPoundMarker.g,
                                    Config.JPoundMarker.b,
                                    100,
                                    false,
                                    true,
                                    2,
                    ...
                    if distance < Config.DrawDistance then
                        ...
                        if Config.PoundMarker.Type ~= -1 then
                            DrawMarker(
                                ...
                                0.0,
                                0.0,
                                Config.PoundMarker.x,
                                Config.PoundMarker.y,
                                Config.PoundMarker.z,
                                Config.PoundMarker.r,
                                Config.PoundMarker.g,
                                Config.PoundMarker.b,
                                100,
                                false,
                                true,
                                2,
            ...
            if CurrentAction then
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    elseif CurrentAction == "ambulance_store_point" then
                        if ESX.PlayerData.job and ESX.PlayerData.job.name == "ambulance" then
                            if
                                IsThisModelACar(model) or IsThisModelABicycle(model) or IsThisModelABike(model) or
